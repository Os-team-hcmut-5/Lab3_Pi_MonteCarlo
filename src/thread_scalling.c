...
int main() {
    const long long TOTAL_POINTS = 100000000LL; // 1e8 points
    
    // Expanded thread counts to show fine-grained scaling
    int thread_counts[] = {1, 2, 4, 8, 12, 16, 20, 24, 32, 48, 64, 128, 256, 512, 1024};
    int num_configs = sizeof(thread_counts) / sizeof(thread_counts[0]);
    double t_single = 0.0;

    printf("Thread Scaling Experiment (N = %lld)\n", TOTAL_POINTS);
    printf("---------------------------------------------------------------------------\n");
    printf("%-10s | %-12s | %-15s | %-10s\n", "Threads", "Pi Estimate", "Time (s)", "Speedup");
    printf("---------------------------------------------------------------------------\n");
    ...
    for (int t = 0; t < num_configs; t++) {
        ...
        if (N == 1) t_single = execution_time;
